...
const { apkpremium } = require('./database/apkpremium')
const { pack+18 } = require('./database/pack+18')
...
prefix = "."
name = "~ DKOFC | BOT"
rdaftar = "OBRIGADO POR SE REGISTAR PARA SER UM MEMBRO DO 𝙏𝙧𝙖𝙨𝙝ф𝘿𝙠𝘽𝙊𝙏😁"
rmenu = "𝙏𝙧𝙖𝙨𝙝ф𝘿𝙠𝘽𝙊𝙏"
limitt = 10
ban = []
userpremium = ["558494740630@s.whatsapp.net"]
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["558494740630@s.whatsapp.net"] // owner number ubah aja
		...
		case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('vocês já são amigos do 𝙏𝙧𝙖𝙨𝙝ф𝘿𝙠:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
					...
					captionnya = `╭─「 *CADASTRO DO USUÁRIO* 」\`\`\`\n│ O registro foi feito com sucesso com SN:\n│ TM08GK8PPHBSJDH10J\`\`\`\n│\n│\`\`\`Pada ${date} ${time}\`\`\`\n│\`\`\`[Nome]: ${pushname2}\`\`\`\n│\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n│\`\`\`user bot\`\`\`\n│\`\`\`Por favor\`\`\`\n│\`\`\`enviar ${prefix}help/menu\`\`\`\n│\`\`\`\n│Total Do utilizador: ${user.length} Orang\`\`\`\n╰─────────────────────────`
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```Lista Banido``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```Lista de Premium 𝙏𝙧𝙖𝙨𝙝ф𝘿𝙠𝘽𝙊𝙏``` :\n'
				...
				case 's':
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`\`\`\`falhou, no momento da conversão ${tipe}para o adesivo stiker\`\`\``)
							})
							.on('end', function () {
								console.log('Finish')
								buff = fs.readFileSync(ran)
						...
						reply(`Envie a imagem com a legenda ${prefix}sticker ou a tag de imagem que foi enviada`)
					}
					await limitAdd(sender) 
					break 
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break  
				...
				case 'testime':
					setTimeout( () => {
					client.sendMessage(from, 'Tempo limite', text, {quoted: mek}) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '5 segundos', text, {quoted: mek}) // ur cods
					}, 5000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '10 segundos', text, {quoted: mek}) // ur cods
					}, 0) // 1000 = 1s,
					break 
				case 'animecry':
					cry = getRandom('.gif')
				...
				if (!isUser) return reply(mess.only.userB)
					...
					const can =['10% muito "cuidado com você: v \nCanda Cuidado: v ',' 30% O espírito do Kaká cuida de si mesmo> <',' 20% O espírito do Kaká cuida de si mesmo ',' 40% Wahh Kaká> <', '50% kaka is beautiful deh> <', '60% Hi Beautiful🐊', '70% Hi Ukhty🐊', '62% Hi']
					const tik = can[Math.floor(Math.random() * can.length)]
					client.sendMessage(from, 'Pertanyaan : Cantik Cek Kakak *'+cantik+'*\nPorcentagem de lindeza : '+ tik +'', text, { quoted: mek })
					break
				case 'ban':
					client.updatePresence(from, Presence.composing) 
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				...
				if (!isUser) return reply(mess.only.userB)
					watak = body.slice(1)
					wa =["compassivo","generoso","rabugento","indulgente","obediente","gentil","baperan","bondoso","paciente","Uwu"," top, de qualquer maneira","gosta de ajuda"]
				...
				if (!isUser) return reply(mess.only.userB)
					hobby = body.slice(1)
					hob =["Cooking","Helping Atok","Mabar","Nobar","Social Media","Helping Others","Watching Anime","Watching Drakor","Riding a Motorcycle","Singing","Dancing","Libra","Desenho","Pouco claro"]
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                     const cfrr = body.slice(8)
                      if (cfrr.length > 300) return client.sendMessage(from, 'Desculpe, o texto é muito longo, máximo de 300 textos', text, {quoted: mek})
                        var nomor = mek.participant
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ marcando todo mundo fds *${pushname2}* ✪══`+ teks +'╚═══〘 𝙏𝙧𝙖𝙨𝙝ф𝘿𝙠𝘽𝙊𝙏〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sucesso deleta todo o chat 𝙏𝙧𝙖𝙨𝙝ф𝘿𝙠𝘽𝙊𝙏\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *𝙏𝙧𝙖𝙨𝙝ф𝘿𝙠𝘽𝙊𝙏 BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão de sucesso')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Não registrado no banco de dados *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[DKOFCBOT]','aqua'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
